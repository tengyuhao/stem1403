...
def result():
    if usernameE.get().strip() == username:
        if passwordE.get().strip() == password:
...
titleL = Label(root, text="Login Form", font=("SF Pro Display", 20, "bold"), padx=10, pady=15).grid(row=0, columnspan=2)

sep1 = Separator(root, orient=HORIZONTAL)
sep1.grid(sticky=N, row=1, columnspan=2, pady=5)
...
btn_login.grid(row=4, column=1, sticky="w", padx=10, pady=10)

btn_quit = Button(root, text="Quit", font=("SF Pro Display", 15), command=root.destroy)
btn_quit.grid(row=4, column=2, sticky="we", padx=10, pady=10)

sep2 = Separator(root, orient=HORIZONTAL)
sep2.grid(columnspan=2)
