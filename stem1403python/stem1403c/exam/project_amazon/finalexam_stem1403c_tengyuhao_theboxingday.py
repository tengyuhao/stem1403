...
Coded by Yuhao Teng,  All rights reserved.
Version 2.0.0
...
class AddressSystem:
    ...
    def __init__(self, apt_no, number, street,  city, province, postalcode):
        self._number = number
        self._street = street
        self._apt_no = apt_no
        self._city = city
...
class OrderSystem:
    ...
    def placeOrder(self, receiver_name, customer_name, shipping_address, billing_address):
        ...
        print(f"""New Message from {self.company_name}: Order-{self.order_number} has been placed.""")
    ...
    def printOrder(self):
        ...
        if self.shipping_status == Status.SHIPPING_NOT_READY:
            title += "We're packaging your order. Once your items have been dispatched, \nwe will send you an email notification.\n"
        elif self.order_status == Status.ORDER_UNPAID:
            title += "Once you've paid your order, we will send you an email notification.\n"
        elif self.shipping_status == Status.SHIPPING_SHIPPED:
            title += "Your shipment is on its way.\n"
        elif self.order_status == Status.ORDER_PAID and self.shipping_status == None:
            title += "We've receive your payment. We're processing your order. \n" \
                     "Once your items have been dispatched, we will send you an email notification.\n"
        elif self.shipping_status == Status.SHIPPING_READY:
            title += "Your order is Ready to ship. Once your items have been dispatched, \nwe will send you an email notification.\n"
...
addressJack = AddressSystem(3, 1500, "Sherbrooke Est", "Quebec City", "Quebec", "K1B 2H6")
addressJack = addressJack.getAddress()
# Order 1
print("Order 1")
order1 = OrderSystem()
...
order1.setOrderTime("20:00:00")
order1.setOrderDate("12-26-2021")
order1.checkout("Visa Online")
order1.packaging()
order1.readyToShip()
order1.shipping()
order1.printOrder()
...
order2 = OrderSystem()
article4 = OrderItemSystem(1, "401002", "Chocolate", "12pieces 360g", 9.99)
article5 = OrderItemSystem(2, "502003", "Game disks", "Call of Duty, Elder Scroll", 11.99)
article4 = article4.getItemInfo()
article5 = article5.getItemInfo()
# print(article1)
order2.addToCart(article4)
order2.addToCart(article5)
order2.placeOrder("Jack", "Peter", addressJack, addressPeter)
order2.setOrderTime("20:00:00")
order2.setOrderDate("12-26-2021")
order2.printOrder()
# order2.checkStatus()
order2.checkout("Visa Online")
order2.printOrder()
order2.packaging()
order2.printOrder()
order2.readyToShip()
